...
流程：Router 分类并选择一个主责子 Agent → ReAct 执行 → 生成主/次意图元数据 → 持久化。
次要意图当前用于可观测输出和评估，不会额外并行调度其他子 Agent。
"""
...
class MultiAgentOrchestrator:
    ...
    def chat(self, user_input: str) -> CustomerServiceResponse:
        ...
        # 与单 Agent 共用确定性多标签元数据策略。路由已经完成，因此这里记录的
        # secondary_intents 不会再次调度子 Agent，避免重复工具调用与回复冲突。
        ...
        # SubAgent 已写入最终正文，避免再保存一份包含完整 reply 的结构化 JSON。
