...
_UNSUPPORTED_PATTERNS = (
    ...
    (r"快递员.*(?:电话|手机号|手机号码|身份证)", "不提供配送人员隐私信息"),
    (r"(?:下周|下月|下个月|下季度|明年|下一版|下一次|下次|重新补货).*(?:出|推出|补货|发布|更新|会不会|多少|哪天|什么|价格|码)", "静态知识库不能预测未来信息"),
    (r"(?:预测|预计).*(?:未来|下周|下月|下个月|价格|促销|最低价)", "静态知识库不能预测未来信息"),
    (r"(?:今晚|今天|现在).*(?:几点|排队|多少(?:台|张|个)|便宜多少|发券)", "需要实时系统而非静态知识库"),
    (r"(?=.*(?:京东|淘宝|天猫|拼多多|官方店|其他平台))(?=.*(?:同款|便宜|价格|多少钱))", "知识库不含外部平台实时价格"),
    (r"具体激活日期|最终会判定|一定出库", "需要订单、品牌或履约实时系统"),
    (r"(?:用户|账号|账户|会员|张三|李四).*(?:用了|使用了|消费了|一共).*(?:多少)?积分", "需要用户账户实时数据"),
)
