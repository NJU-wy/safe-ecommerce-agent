...
class SkillManager:
    ...
    def build_catalog_prompt(self) -> str:
        ...
        lines = [
            "\n\n## 可用技能（Skills）",
            "以下技能仅用于复杂、异常或用户明确要求完整标准流程的任务。",
            "订单、物流、商品、政策和退款等直接问题已有明确工具规则，禁止为这些常规问题加载技能。\n",
        ]
        ...
        lines.append("\n### 技能使用方式")
        lines.append("1. 默认直接使用业务工具，不调用 load_skill")
        lines.append("2. 仅在复杂异常、多步骤编排或用户明确要求标准流程时加载一次")
        lines.append("3. 同一会话已加载过的技能不得重复加载")
