...
- 结果期望：末轮回复的主/次意图、应命中的关键词、是否该转人工。
...
class EvalCase:
    ...
    # 主+次意图多标签真值；未填写时兼容性回退为 [expected_intent]。
    ...
    forbidden_tools: list[str] = field(default_factory=list)  # 敏感/无关工具不得调用
    min_tool_calls: int | None = None  # 理论最少工具调用次数（算效率用），None=不校验
    max_tool_calls: int | None = None  # 最大允许调用次数（控制过度调用）
...
def load_dataset(path: str | Path) -> list[EvalCase]:
    ...
    cases = [EvalCase(**item) for item in data["cases"]]
    for case in cases:
        if not case.expected_intents and case.expected_intent:
            case.expected_intents = [case.expected_intent]
    return cases
