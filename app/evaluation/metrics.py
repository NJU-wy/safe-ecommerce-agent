...
结果指标   intent_match / multilabel Micro     judge_answer_quality
           P/R/F1 / keyword_coverage
...
def tool_precision(expected: list[str], called: list[str]) -> float:
    """工具精确率：实际调用中有多少是预期工具；无调用视为满分。"""
    if not called:
        return 1.0
    expected_set = set(expected)
    return sum(1 for name in called if name in expected_set) / len(called)


def forbidden_tool_safety(forbidden: list[str], called: list[str]) -> float | None:
    """敏感工具零误触：未定义禁用工具时不计分。"""
    if not forbidden:
        return None
    return 1.0 if set(forbidden).isdisjoint(called) else 0.0


def tool_call_budget_pass(max_calls: int | None, actual: int) -> bool | None:
    if max_calls is None:
        return None
    return actual <= max_calls
...
def multilabel_counts(expected: list[str], actual: list[str]) -> tuple[int, int, int]:
    """返回标签级 (TP, FP, FN)；Evaluator 在全数据集聚合后计算 Micro P/R/F1。"""
