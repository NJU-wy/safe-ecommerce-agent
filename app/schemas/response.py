...
class CustomerServiceResponse(BaseModel):
    ...
    `intent` 保留给旧调用方；新调用方应读取 `primary_intent` 与
    `secondary_intents`。当前多标签只描述用户诉求，不代表同时调度多个子 Agent。
    """

    intent: IntentType = Field(description="识别到的用户意图类型")
    primary_intent: Optional[IntentType] = Field(
        default=None, description="主意图；intent 为向后兼容别名"
    )
    secondary_intents: list[IntentType] = Field(
        default_factory=list, description="同一轮中识别到的次要意图，按业务优先级排列"
    )
