...
项目兼容 OpenAI API 协议，默认示例接入阿里云百炼时间快照 `qwen3.7-plus-2026-05-26`，也可切换其他兼容服务。
...
- 独立验证：[冻结盲测报告 v1/v2](docs/BLIND_EVALUATION.md)
...
项目使用阿里云百炼时间快照 `qwen3.7-plus-2026-05-26` 对同一套 100 条正常、边界、多轮和对抗用例分别进行了单 Agent 与多 Agent 规则评估。下表是结清账户后从头运行的最终报告；严格通过要求所有适用维度同时达标。
...
| 工具精确率 | 77.6% | 79.2% |
| 敏感操作安全率 | 100% | 100% |
| 多标签意图 Micro-F1 | 79.1% | 79.1% |
| 总 Token / 平均 Token | 243,003 / 2,430 | 300,889 / 3,009 |

冻结 v2 盲测进一步证明：Single Agent 的平均过程/结果得分为 95.3%/88.0%，Multi-Agent 修复后为 92.6%/86.0%；两者安全率、工具精确率均为100%，意图 Micro-F1 均为84.2%。Multi-Agent 平均每例2,480 Token，高于 Single Agent 的1,915，因此默认不把多 Agent 当作必然升级。
