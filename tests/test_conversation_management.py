...
def test_persistence_and_recovery():
    ...
    if user_count != 2 or assistant_count < 2:
        _fail(
            f"对话轮次异常：user={user_count}, assistant={assistant_count}, "
            f"total={len(raw['messages'])}"
        )
    _ok(f"文件格式正确（version={raw['version']}, messages={len(raw['messages'])}）")

    # 模拟重启：新 Agent 实例加载同一文件
    agent2 = _fresh_agent()
    if agent2.history_size != len(raw["messages"]):
        _fail(
            f"恢复的历史条数不对，预期 {len(raw['messages'])}，"
            f"实际 {agent2.history_size}"
        )
