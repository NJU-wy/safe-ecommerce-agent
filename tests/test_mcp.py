...
def test_mcp_connection():
    ...
    try:
        ...
        if len(tools) != len(EXPECTED_MCP_TOOLS):
            _fail(f"期望 {len(EXPECTED_MCP_TOOLS)} 个工具，实际发现 {len(tools)} 个")
...
def test_tool_manager_mcp():
    ...
    try:
        ...
        if {d["function"]["name"] for d in defs} != EXPECTED_ALL_TOOLS:
            _fail(f"聚合工具集合不正确，实际 {len(defs)} 个")
...
def test_tool_manager_local():
    ...
    try:
        defs = manager.tool_definitions
        if {d["function"]["name"] for d in defs} != EXPECTED_ALL_TOOLS:
            _fail(f"本地工具集合不正确，实际 {len(defs)} 个")
        _ok("本地模式加载 4 个工具")
